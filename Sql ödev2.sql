...
select o.order_date, o.ship_city,UPPER( e.first_name || ' ' || e.last_name) as fullname from orders o join employees e on e.employee_id = o.employee_id
where date_part('year', o.order_date) = 1997

34. 1997 yılında sipariş veren müşterilerin contactname i, ve telefon numaraları ( telefon formatı 2223322 gibi olmalı )

SELECT c.contact_name, 
       regexp_replace(c.phone, '[^0-9]', '', 'g') AS formatted_phone
FROM orders o
...
select c.category_name,p.product_name,od.quantity from orders o 
...
select c.category_name,p.product_name,od.quantity from orders o join order_details od on o.order_id = od.order_id
join products p on p.product_id = od.product_id
join categories c on p.category_id = c.category_id
join suppliers s on s.supplier_id = p.supplier_id
where o.order_id = 10248
...
select e.employee_id, e.first_name || ' ' || e.last_name as ad_soyad from employees e 
where e.employee_id = (select o.employee_id from orders o join order_details od on o.order_id = od.order_id
...
select e.employee_id, e.first_name, e.last_name from employees e where e.employee_id = (select o.employee_id from orders o 
join order_details od on o.order_id = od.order_id
where EXTRACT(year from o.order_date) = 1997
...
select c.category_name,s.company_name,p.product_name from products p 
